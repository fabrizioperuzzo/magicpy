...
def retrieve_stock(stock, i, ist_dict):
    name_ist = 'ist'+str(i)
    ist_dict[name_ist] = Yahoo_Scan(stock,'01/01/2006','d',name_ist)
    ist_list.append(name_ist)
    print('\nfinished istance: ', name_ist,' stock name: ', stock)

thread_list = []

for i,stock in enumerate(symb):
    t = Thread(target=retrieve_stock, args=(stock,i,ist_dict))
...
# check if ist_dic == ist_list
print('\n **** check if ist_dic == ist_list   ------>    ', len(list(ist_dict.keys())) == len(ist_list))

df1 = pd.DataFrame({})

for name_ist in ist_dict.keys():
